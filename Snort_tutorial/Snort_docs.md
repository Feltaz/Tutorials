...
# Dependencies   
Most software (snort included) depend on some external libraries and packages that developpers reuse for specific purposes in order to not rewrite code that has already  been written by someone else.  
...
```  
Detecting Port scans is very important because most likely the attacker will move on to the next step gaining additional information about the discovered service Without knowing that we already detected his presence the second he initiated that scan.  
...
# Conclusion 
Snort is open source and free to use by anyone, you can even use community and registered users rules FOR FREE which include UP to date rules to protect your network and hosts. You can also pay a small fee  to get subscribers rules that are updated every two weeks or so. (registered users rules are the same as subscriber rules but with a 30 days delay). Protecting your network has become a must since the threat of unknown attacks to your organisation is growing each day in a very rapid manner, therefore a preventative solution might save you from the catastrophe of a cyber attack one day, Take action now and protect your systems!